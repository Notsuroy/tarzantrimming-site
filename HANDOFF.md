...
| 2026-06-09 | Custom 404 page (cron, commit pending) | **P4/UX deliverable.** No 404 page existed, so any unmatched URL returned the homepage at HTTP 200 (CF Pages soft-404 fallback per `feedback_cloudflare_pages_unknown_url_serves_home.md`) — bad for SEO (soft 404s) and gives lost visitors no recovery path. Added `src/pages/404.astro` (BaseLayout, brand-coherent: cream bg, forest H1 "This branch leads nowhere.", 6 recovery cards linking home + both service detail pages + services menu + about + contact, "Back to home" button). Astro built `dist/404.html` (7.2KB); CF Pages now serves it with a proper 404 status for unmatched routes. Build clean 7pg; dist sweep em=0/en=0/24-7=0; all 6 links resolve to real built pages. Shipped via wrangler direct-upload (GitHub auto-deploy still not connected). Also folded in the T04:14 P2 run's uncommitted HANDOFF changelog edits (the `7b80ac9` service-detail-pages doc rows that prior run left dirty in the working tree). |
